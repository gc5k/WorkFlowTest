...
dat=read.csv("./data/20200721all_cgb.csv", as.is = T, header = T)
...
```{r, Baseline}
BS=c("AgeOfOnset","SiteOfOnset","DelayDays","DelayMonths","heightpre","	weightpre","weightdiagnosis","BMIpre","BMIdiagnosis","ALSFRSRDiagnosis","chWeightPre", "chBMIPre","chFSPre")

BSidx=c()
for(i in 1:length(BS)) {
  BSidx=c(BSidx,which(rownames(SUMmat) == BS[i]))
}

knitr::kable(SUMmat[BSidx,c(1,3,5,7,8)], caption = "Baseline characteristics") %>%
kable_styling("striped", full_width = T)
#row_spec(row=16:16, color="white", background="red")
